...
namespace Esri.ArcGISRuntime.Toolkit.UI.Controls
{
    ...
    public partial class TraceConfigurationsView : Control
    {
        ...
        {
            get => _utilityNetworks;
        }

        /// <summary>
        /// Gets a list of named trace configurations for a specific utility network.
        /// </summary>
        ...
        private string GetStatusBasedOnSelection()
        {
            ...
            else
            {
                ...
                stringBuilder.AppendLine($"The '{SelectedTraceConfiguration.Name}' trace requires at least '{minimum}' starting location(s).");
        ...
        private void DeleteStartingLocation(UtilityElement utilityElement)
        {
            GeoView.DismissCallout();
            _startingLocations.Remove(utilityElement);
            Graphic startingLocationGraphic = _traceLocationsGraphicsOverlay.Graphics.Where(g => g.Attributes.ContainsKey(utilityElement.GlobalId.ToString())).First();
            _traceLocationsGraphicsOverlay.Graphics.Remove(startingLocationGraphic);
        }

        private async void ShowStartingPoint(UtilityElement utilityElement)
        {
            GeoView.DismissCallout();
            CalloutDefinition calloutDefinition = new CalloutDefinition(utilityElement.AssetGroup.Name);
            calloutDefinition.DetailText = utilityElement.AssetType.Name;
            var features = await SelectedUtilityNetwork.GetFeaturesForElementsAsync(new UtilityElement[] { utilityElement });
            if (features.Any())
            {
                MapPoint location = new MapPoint(features.First().Geometry.Extent.XMax, features.First().Geometry.Extent.YMax);
        ...
        private async void OnGeoViewTapped(object sender, GeoViewInputEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    geoView.DismissCallout();
                    var identifyResults = await geoView.IdentifyLayersAsync(e.Position, 10d, false);
                    foreach (var identifyResult in identifyResults)
                    {
                        if (GetFeature(identifyResult) is ArcGISFeature feature && SelectedUtilityNetwork.CreateElement(feature) is UtilityElement element)
                        {
                            ...
                            bool startingLocationExists = _startingLocations.Where(loc => loc.GlobalId.Equals(element.GlobalId)).Count() > 0;
        ...
        private async void OverrideStartingLocations(ObservableCollection<UtilityElement> overrideList)
        {
            Reset();
            foreach (UtilityElement element in overrideList)
            {
                var features = await SelectedUtilityNetwork.GetFeaturesForElementsAsync(new UtilityElement[] { element });
                if (features.Any())
                {
                    MapPoint location = new MapPoint(features.First().Geometry.Extent.XMax, features.First().Geometry.Extent.YMax);
                    _traceLocationsGraphicsOverlay.Graphics.Add(new Graphic(location,
                                new KeyValuePair<string, object>[] { new KeyValuePair<string, object>(element.GlobalId.ToString(), element.AssetGroup.Name) },
                                StartingLocationSymbol ?? _defaultStartingLocationSymbol));
                    _startingLocations.Add(element);
                }
